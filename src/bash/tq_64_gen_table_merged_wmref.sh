...
cleanup() {
    ...
    if [[ "${status}" -eq 0 ]] && [[ "${cache}" = false ]]; then
        rm ${subjectdir}/tmp_ROI-SUVR_merged_wmref.csv
...
# Make CSV
echo "${ID}" > ${subjectdir}/tmp_ROI-SUVR_merged_wmref.csv

# aseg+brainstem
fslmaths ${merged} -uthr 999.5 ${subjectdir}/tmp_merged_0000.nii.gz
fslstats -K ${subjectdir}/tmp_merged_0000.nii.gz ${suvr_wmref} -M > ${subjectdir}/tmp_pet_suvr_wmref_roi-suvr_0000
cat ${subjectdir}/tmp_pet_suvr_wmref_roi-suvr_0000 | sed -n '7,8p;10,13p;17,18p;26p;28p;46,47p;49,54p;58p;60p;173,175p;251,255p' >> ${subjectdir}/tmp_ROI-SUVR_merged_wmref.csv
...
for num in 1000 2000 3000 4000; do
    ...
    fslstats -K ${subjectdir}/tmp_merged_${num}.nii.gz ${suvr_wmref} -M > ${subjectdir}/tmp_pet_suvr_wmref_roi-suvr_${num}
    cat ${subjectdir}/tmp_pet_suvr_wmref_roi-suvr_${num} | sed -n '1,3p;5,9p;11,13p;15,18p;21,22p;24,25p;28,31p;33,35p' >> ${subjectdir}/tmp_ROI-SUVR_merged_wmref.csv
done

paste -d ',' ${TAMEQDIR}/lib/colheader_merged.txt ${subjectdir}/tmp_ROI-SUVR_merged_wmref.csv > ${subjectdir}/ROI-SUVR_merged_wmref.csv
