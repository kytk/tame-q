...
do
  ...
  fslstats -K ${wmparc}_r_0000.nii.gz $f -M > tmp_${fsid}_suvr_wmparc_merged_mean_0000
  cat tmp_${fsid}_suvr_wmparc_merged_mean_0000 | sed -n '7,8p;10,13p;17,18p;26p;28p;46,47p;49,54p;58p;60p;251,255p' >> ${fsid}_pmpbb3_suvr_wmparc_merged_mean.tsv
  
  fslstats -K ${bsseg}_r.nii.gz $f -M > tmp_${fsid}_suvr_bsseg_mean
  cat tmp_${fsid}_suvr_bsseg_mean | sed -n '173,175p;178p' >> ${fsid}_pmpbb3_suvr_wmparc_merged_mean.tsv
...
rm colheader_merged.txt tmp_*_suvr_wmparc_merged_mean_?000 tmp_*_suvr_bsseg_mean *_r_?000.nii.gz

echo "Done. Please check suvr_wmparc_merged_${timestamp}.tsv"
